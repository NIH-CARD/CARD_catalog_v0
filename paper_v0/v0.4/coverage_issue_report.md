...
| mtDNA_AD (Mitochondrial DNA in Aging and AD) | Klein, Trumpff, ... Picard et al., medRxiv 2021 | https://doi.org/10.1101/2021.05.20.21257456 | **BLOCKED** | Exact match on sample size (1,361), source cohorts (ROSMAP/Mayo/MSBB); `[X]` because claim was phrased around catalog's own name, which the paper never uses |
| VirusResilience_LCL | Smullen et al. 2023, Sci Rep | https://pubmed.ncbi.nlm.nih.gov/37369829/ | **BLOCKED** | Exact match on donor count (60), allele split, rare 3-sample GCT detail, cell source; same claim-wording block as above |
...
| AUSBB (Australian Brain Bank snRNA-seq study) | — | — | No candidates | Exhaustive search (synapse ID, brain bank names, abbreviation, consortium name) found nothing; likely too recently deposited to have a paper yet |
| ADMC-ADNI Metabolon | — | — | No candidates | Several ADNI+metabolomics papers found and actively ruled out as different ADMC sub-platforms (targeted p180, Biocrates Q500, lipidomics) |

**Yield: 8/12 resources (67%) got a verified match** (true recall closer to 10/12 once the claim-wording issue is fixed). **Precision: pending your spot-check above.**

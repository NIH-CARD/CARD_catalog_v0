...
ground-truth JSON output. Designed to run against large ID lists (100k+)
with checkpointing so it can be interrupted and resumed.
...
Usage
  python scrapers/scrape_annotations.py \\
      --input k8s/input/article_ids_eval.csv \\
      --output scrapers/output/annotations_eval.json \\
      [--batch-size 8] [--pause 0.2] [--retries 3] [--resume]
"""
...
def parse_response(api_response: list) -> dict[str, list]:
    ...
    Preserves all annotation types (Diseases, Gene_Proteins, Accession Numbers,
    etc.). Callers wanting a narrower view can pipe the result through
    :func:`extract_accession_numbers`.
    """
    result = {}
    for article_entry in api_response:
        ...
        result[pmcid] = list(article_entry.get("annotations", []))
    return result
...
def fetch_annotations(
    ...
    Saves a checkpoint every 1000 batches so large runs can be resumed.
    """
    if done is None:
        done = {}
    ...
    for i, chunk in enumerate(batch_list(remaining, batch_size)):
        ...
        # Build URL manually to avoid requests encoding the colon in PMC:PMCxxxxx
        url = f"{ANNOTATIONS_URL}?articleIds={api_ids}"

"""
Resources page - Browse and explore neuroscience research resources.
...
# Page config
st.set_page_config(
    page_title="Resources - CARD Catalog",
    page_icon="📊",
    layout="wide"
)
...
def main():
    ...
    st.markdown(
        "Explore neuroscience and brain disorder research resources with interactive filtering and visualization."
    )
    ...
    if df.empty:
        st.error("No resource data available. Please check data files.")
        return
    ...
    # Display count
    st.info(f"Showing {len(filtered_df)} of {len(df)} Resources")
    ...
    with tab1:
        ...
        if filtered_df.empty:
            st.warning("No Resources match the current filters.")
        else:
    ...
    # Tab 2: Browse Resources
    with tab2:
        st.subheader("Resources Catalog")
        ...
        if filtered_df.empty:
            ...
            for idx, row in filtered_df.iterrows():
                with st.expander(f"**{row.get('Resource Name', 'Unknown')}** ({row.get('Abbreviation', 'N/A')})"):
    ...
    with tab3:
        ...
        if filtered_df.empty:
            st.warning("No resources to visualize.")
        else:
            ...
            with col1:
                connection_features = st.multiselect(
                    ...
                    help="Resources will be connected if they share values in selected features"
                )
            ...
            if not connection_features:
                ...
                with st.spinner("Building knowledge graph..."):
                    ...
                    col1.metric("Resources", stats["num_nodes"])
                    ...
                    if stats["num_nodes"] > 0:
                        ...
                        if stats["top_nodes"]:
                            st.markdown("#### Most Connected Resourcesß")
                            top_df = pd.DataFrame(
                                stats["top_nodes"],
    ...
    with tab4:
        ...
        if filtered_df.empty:
            st.warning("No resources to analyze.")
        else:
            st.info(f"Ready to analyze {len(filtered_df)} of {len(df)} resources ({len(filtered_df)/len(df)*100:.1f}% of catalog)")

            col1, col2 = st.columns([3, 1])

            with col2:
                if st.button("🤖 Analyze Resources", type="primary"):
    ...
    with tab5:
        ...
        if filtered_df.empty:
            st.warning("No resources to export.")
        else:
            st.markdown(f"**Export {len(filtered_df)} filtered resources**")
            ...
            with col1:
                ...
                st.download_button(
                    ...
                    mime="text/csv",
                    help="Export filtered resources as CSV file"
                )

                st.download_button(
                    label="📥 Export as TSV",
                    data=export_dataframe_tsv(filtered_df),
                    file_name="resources_export.tsv",
                    mime="text/tab-separated-values",
                    help="Export filtered resources as TSV file"
                )

                st.download_button(
                    label="📥 Export as JSON",
                    data=export_dataframe_json(filtered_df),
                    file_name="resources_export.json",
                    mime="application/json",
                    help="Export filtered resources as JSON file"
                )

            with col2:
                st.markdown("#### Export as Text Summary")

                summary_text = export_text_summary(filtered_df, "CARD Resources Summary")

                st.download_button(
                    label="📥 Export Summary as Text",
                    data=summary_text,
                    file_name="resources_summary.txt",
                    mime="text/plain",
                    help="Export filtered resources as formatted text"
                )
                ...
                if SESSION_KEYS["graph_data"] in st.session_state:
                    ...
                    st.download_button(
                        ...
                        data=graph_summary,
                        file_name="resource_graph_summary.txt",
                        mime="text/plain",
